...
class PurchasesObserver implements AdaptyUIPaywallsEventsObserver, AdaptyUIOnboardingsEventsObserver {
    ...
    try {
      ...
      if (!isActivated) {
        await Adapty().activate(
          configuration: AdaptyConfiguration(apiKey: 'YOUR_ADAPTY_KEY')
     ...
     // await callGetPaywallForDefaultAudience('example_ab_test');
      await callGetPaywallForDefaultAudience('SailPro_Paywall_AB');
    } catch (e) {
  ...
  Future<void> _setFallbackPaywalls() async {
    final assetId =  'assets/android_3_8_0_fallback.json'; //Platform.isIOS ? 'assets/ios_3_8.0_fallback.json' :
